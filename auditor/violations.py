...
def bad_winds(winds,maxwind,maxcross):
    ...
    if 'gusts' in winds:
        if winds['speed'] > winds['gusts']:
            winds['speed'] = winds['speed']
        else:
            winds['speed'] = winds['gusts']

    knots = 1.94384
    
    if winds['units'] == 'MPS':
        winds['crosswind'] = winds['crosswind'] * knots
        winds['speed'] = winds['speed'] * knots
...
def get_weather_report(takeoff,weather):
    ...
    takeoff2 = takeoff.isoformat()
    if takeoff2 in weather:
        return weather[takeoff2]
    elif takeoff2 not in weather:
        for x in weather:
            curr = x
            recent = utils.str_to_time(curr, takeoff)
            if recent < takeoff:
                return weather[x]
    else:
        return None
