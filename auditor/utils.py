...
def str_to_time(timestamp,tzsource=None):
    ...
    if t.tzinfo is None and tzsource is not None:
        if type(tzsource) == str:
            return timezone(tzsource).localize(t)
        else:
            return t.replace(tzinfo=tzsource.tzinfo)
    else:
        return t
...
def daytime(time,daycycle):
    ...
    try:
        date = time.isoformat()
        date1 = str_to_time(date, daycycle['timezone'])
        y = date1.strftime('%Y')
        md = date1.strftime('%m-%d')
        sunrise = daycycle[y][md]["sunrise"]
        sunset = daycycle[y][md]["sunset"]
        sunrise_tz = str_to_time(y+'-'+md+' '+sunrise,daycycle['timezone'])
        sunset_tz = str_to_time(y+'-'+md+' '+sunset,daycycle['timezone'])
        return sunrise_tz < date1 < sunset_tz
    except:
        return None
