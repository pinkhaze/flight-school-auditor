...
def get_certification(takeoff,student):
    ...
    try:
        pos3 = utils.str_to_time(student[3],takeoff)
        pos4 = utils.str_to_time(student[4],takeoff)
        pos5 = utils.str_to_time(student[5],takeoff)
        pos6 = utils.str_to_time(student[6],takeoff)
        if pos3 is not None and takeoff < pos3:
            c = PILOT_INVALID
        if pos3 is not None and takeoff > pos3:
            c = PILOT_NOVICE
        if pos4 is not None and takeoff > pos4:
            c = PILOT_STUDENT
        if pos5 is not None and takeoff > pos5:
            c = PILOT_CERTIFIED
        if pos6 is not None and takeoff > pos6:
            c = PILOT_50_HOURS
        return c
    except:
        return c
...
def get_best_value(data, index, maximum=True):
    ...
    val = float(data[0][index])
    for row in range(1,len(data)):
        if maximum == True:
            if float(data[row][index]) > val:
                val = float(data[row][index])
        else:
            if float(data[row][index]) < val:
                val = float(data[row][index])
    return val
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    # Based in Bruce's Step-by-Step Video on the get_minimums function
    # https://lms.ecornell.com/courses/1680382/discussion_topics/8174874
    ...
    for row in minimums[1:]:
        ...
        if col0 and col1 and col2 and col3 == True:
